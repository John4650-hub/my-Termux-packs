...
std::vector<std::string> getAudioFiles(const std::string& folderPath) {
  ...
  std::vector<std::string> validExtensions = { ".mp3", ".flac", ".wav" };
...
Component logsWindow() {
  ...
  public:
    Impl() {
      ...
      Component txtlogs = Container::Vertical({
          Renderer(textarea_log, [textarea_log] {
            return vbox({text("Audio logs"), separator(), textarea_log->Render() | flex | size(HEIGHT, EQUAL, 50)}) | border | bgcolor(Color::Black);
          }),
      });
      Add(txtlogs);
    }
...
int main() {
  ...
  auto windowContainer = Container::Stacked({exitBtn, logout, musicListWindow, audioPlayerWindow});
