...
class MyCallback : public oboe::AudioStreamCallback{
	public:
		...
		oboe::DataCallbackResult onAudioReady(oboe::AudioStream *media,void *audioData, int32_t numFrames) override{
			std::memcpy(static_cast<float*>(audioData),getPcmData(mFormatCtx, mPacket, mDecCtx, mFrame, mSwrCtx,mStream_index)[0],numFrames * media->getChannelCount() * sizeof(float));
